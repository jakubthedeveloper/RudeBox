...
static_assert(AppConfig::Controls::SENS_EFFECTIVE_MAX_LOW >
                      AppConfig::HitDetection::PAD_INPUT_MIN &&
                  AppConfig::Controls::SENS_EFFECTIVE_MAX_HIGH >
                      AppConfig::HitDetection::PAD_INPUT_MIN,
              "Sensitivity effective maxima must exceed the input minimum");
...
float normalizedKnob(float filteredValue, bool invert) {
  float knob = clamp01(filteredValue / 255.0f);
...
float mapPitchDrop(float knob) {
  ...
  return lerp(AppConfig::Controls::PITCH_DROP_MIN_OCTAVES,
              AppConfig::Controls::PITCH_DROP_MAX_OCTAVES, shaped);
}

void applyMappedValue(uint8_t index, float filteredValue,
...
float velocityFromPeak(uint16_t peak, float sensitivity) {
  const float knob = clamp01(sensitivity);
  const float effectiveMax =
      lerp(AppConfig::Controls::SENS_EFFECTIVE_MAX_LOW,
           AppConfig::Controls::SENS_EFFECTIVE_MAX_HIGH, knob);
  const float linearVelocity = clamp01(
  ...
  const float curveExponent =
      lerp(AppConfig::Controls::SENS_CURVE_EXPONENT_LOW,
           AppConfig::Controls::SENS_CURVE_EXPONENT_HIGH, knob);
  return clamp01(powf(linearVelocity, curveExponent));
}
